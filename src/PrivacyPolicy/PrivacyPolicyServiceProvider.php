...
class PrivacyPolicyServiceProvider extends ServiceProvider
{
	...
	protected function mergeConfig()
	{
		$this->mergeConfigFrom(
			__DIR__ . '/../config/privacy-policy.php',
			'privacy-policy'
		);
	}
	...
	{
		$this->publishes([
			__DIR__ . '/../config/privacy-policy.php' => config_path(
				'privacy-policy.php'
			),
		]);
	}
	...
	private function loadTranslations()
	{
		...
		$this->loadTranslationsFrom(
			$vendorDir .
				"/webflorist/privacy-policy-text/dist/php/colon-prefix/$viewpoint",
			'Webflorist-PrivacyPolicy'
		);
	}
	...
	private function addRoutes()
	{
		...
		if (config('routetree.api.enabled')) {
			$router->group(
				['prefix' => config('routetree.api.base_path'), 'middleware' => 'api'],
				function (Router $router) {
					$router
						->resource('routes', RoutesController::class)
						->only(['index', 'show']);
				}
			);
		}
		...
		if (config('routetree.sitemap.route.enabled')) {
			$router->get(config('routetree.sitemap.route.path'), [
