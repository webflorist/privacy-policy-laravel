...
class PrivacyPolicy
{
	...
	{
		$this->allProcessors = array_merge(
			config('privacy-policy.processors'),
			\Webflorist\PrivacyPolicyText\Helpers::getProcessors()
		);

		// Determine used processors and interpolations for translation
		foreach (
			config('privacy-policy.data_processing')
			as $processType => $process
		) {
			$processors = is_array($process['processor'])
				? $process['processor']
				: [$process['processor']];
			...
			foreach ($processors as $processorKey) {
				if (!isset($this->allProcessors[$processorKey])) {
					throw new \Webflorist\PrivacyPolicy\Exceptions\ProcessorNotFoundException(
						"Processor '$processorKey' used for data-processing '$processType' not found in processor-list. Please state processor details via the config key 'privacy-policy.processors'."
					);
				}
			...
			$this->interpolations[$processType . '_processor'] = implode(
				', ',
				$processorLinks
			);
			$this->interpolations[$processType . '_service'] =
				$process['service'] ??
				__(
					"Webflorist-PrivacyPolicy::privacy-policy.data_processing.$processType.title"
				);

			// Put processors in $this->usedProcessors
			foreach ($processors as $processorKey) {
				if (!isset($this->usedProcessors[$processorKey])) {
					$this->usedProcessors[$processorKey] =
						$this->allProcessors[$processorKey];
				}
				...
				$this->usedProcessors[$processorKey]['data_categories'] = array_unique(
					array_merge(
						$this->usedProcessors[$processorKey]['data_categories'],
	...
	{
		return \Webflorist\PrivacyPolicyText\Helpers::renderText(
			__("Webflorist-PrivacyPolicy::privacy-policy.$key", $this->interpolations)
		);
	}
}
